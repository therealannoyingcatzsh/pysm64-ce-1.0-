...
def alpha_rect(surface: pygame.Surface, rect: pygame.Rect, rgb=(0, 0, 0), alpha=128):
    """Draw a semi-transparent rectangle (pygame.draw ignores alpha on the main display)."""
    s = pygame.Surface((rect.width, rect.height), pygame.SRCALPHA)
    s.fill((rgb[0], rgb[1], rgb[2], alpha))
...
def draw_castle(surface, x, y, scale=1.0):
    ...
    base_rect = pygame.Rect(
        iround(x - 40 * sx), iround(y - 30 * sx),
        iround(80 * sx), iround(60 * sx)
    )
    pygame.draw.rect(surface, (200, 200, 220), base_rect, border_radius=iround(5 * sx))

    for i in range(-1, 2):
        tower_x = x + i * 20 * sx
        tower_rect = pygame.Rect(
            iround(tower_x - 10 * sx), iround(y - 60 * sx),
            iround(20 * sx), iround(30 * sx)
        )
        pygame.draw.rect(surface, (180, 180, 200), tower_rect, border_radius=iround(3 * sx))

        top_rect = pygame.Rect(
            iround(tower_x - 8 * sx), iround(y - 70 * sx),
            iround(16 * sx), iround(10 * sx)
        )
        pygame.draw.rect(surface, (220, 220, 240), top_rect, border_radius=iround(2 * sx))

    door_rect = pygame.Rect(
        iround(x - 15 * sx), iround(y - 10 * sx),
        iround(30 * sx), iround(30 * sx)
    )
    pygame.draw.rect(surface, (160, 140, 120), door_rect, border_radius=iround(3 * sx))
...
def draw_level_icon(surface, x, y, course_index, selected=False):
    ...
    if course_index == 0:  # Castle Grounds
        ...
        pygame.draw.rect(surface, theme_color, (x - 25, y, 50, 20))
    ...
    elif course_index == 13:  # Tiny-Huge Island (animate tiny/huge swap)
        t = (pygame.time.get_ticks() // 700) % 2
        size = 26 if t == 1 else 14
        pygame.draw.circle(surface, icon_color, (x, y), size)
        pygame.draw.circle(surface, theme_color, (x, y), max(1, size - 5))
        ...
        pygame.draw.line(surface, theme_color, (x, y), (int(end_x), int(end_y)), 3)
        ...
        for i, color in enumerate(colors):
            r = pygame.Rect(x - 30 + i * 4, y - 20, 8, 40)
            pygame.draw.rect(surface, color, r)

def draw_star(surface, x, y, size, color):
    """Draw a star shape (int points for pygame)."""
    pts = []
    for i in range(5):
        angle = math.pi / 2 + i * 2 * math.pi / 5
        ox = x + size * math.cos(angle)
        oy = y + size * math.sin(angle)
        ix = x + (size / 2) * math.cos(angle + math.pi / 5)
        iy = y + (size / 2) * math.sin(angle + math.pi / 5)
...
def draw_debug_menu():
    ...
    # Course grid (4x4)
    for i, (name, *_colors) in enumerate(COURSES):
        row = i // 4
        col = i % 4
        x = 100 + col * 200
        y = 100 + row * 120
...
def draw_level_view():
    ...
    for i in range(10):
        sx = int((t / 100 + i * 50) % WIDTH)
        sy = int(100 + 50 * math.sin(t / 1000 + i))
        ssize = 5 + 3 * math.sin(t / 500 + i)
        draw_star(screen, sx, sy, ssize, icon_color)

    # Info panel (real transparency)
    panel_rect = pygame.Rect(50, 50, 320, 210)
    alpha_rect(screen, panel_rect, (0, 0, 0), alpha=140)
    ...
    total_seconds = level_time_ms // 1000
    ...
    draw_level_icon(screen, 520, 210, current_course, True)
...
def draw_castle_view():
    ...
    tower_rect = pygame.Rect(cx - 30, cy - 150, 60, 100)
    pygame.draw.rect(screen, (180, 180, 200), tower_rect, border_radius=10)

    tower_top = pygame.Rect(cx - 20, cy - 180, 40, 30)
    pygame.draw.rect(screen, (220, 220, 240), tower_top, border_radius=5)

    for offset in (-80, 80):
        side_tower = pygame.Rect(cx + offset - 20, cy - 100, 40, 50)
        pygame.draw.rect(screen, (190, 190, 210), side_tower, border_radius=8)

    for i in range(3):
        angle = pygame.time.get_ticks() / 2000 + i * 2 * math.pi / 3
        mx = cx + 200 * math.cos(angle)
        my = cy - 100 + 50 * math.sin(angle)
        pygame.draw.circle(screen, (255, 255, 200), (int(mx), int(my)), 15)
...
# ---------------- MAIN LOOP ----------------
def main():
    global cursor, state, current_course, level_time_ms
